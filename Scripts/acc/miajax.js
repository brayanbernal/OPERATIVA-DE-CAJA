...
function addNewRow(consecutivo) {
    ...
    //epara debito
    newTR.append('<td class="nc-oculto" ><input type="text" id="debito' + i2 + '" class="form-control" onkeypress="return IsNumeric(event);"></td>');

    //epara credito
    newTR.append('<td class="nc-oculto" ><input type="text" id="credito' + i2 + '" class="form-control" onkeypress="return IsNumeric(event);"></td>');
...
$(document).on('change', "#tr", function () {
    ...
    var credito = items.find("input[id*='base']").val();
...
function RemoveRow(index)
{
    ...
    //$("#anotaciones").remove($("tr").attr("data-index", index));
}
